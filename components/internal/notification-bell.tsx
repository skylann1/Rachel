...
interface InternalNotificationBellProps {
  initialUnread: number;
  userId?: string;
  mutedTypes?: NotificationType[];
}

export function InternalNotificationBell({ initialUnread, userId, mutedTypes = [] }: InternalNotificationBellProps) {
  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        className={`relative p-2 text-slate-400 hover:text-primary transition-colors ${pulse ? 'animate-pulse' : ''}`}
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            ...
            <Link href="/dashboard/inbox" onClick={() => setOpen(false)} className="text-xs font-bold text-primary hover:text-primary/80">
              Lihat Semua
            </Link>
          </div>
